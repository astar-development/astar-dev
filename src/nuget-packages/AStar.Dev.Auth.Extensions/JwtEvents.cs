...
public sealed class JwtEvents(ILogger<JwtEvents> logger)
{
    ...
    private async Task LogAuthenticationFailureAsync(Send send, string applicationName, AuthenticationFailedContext context)
    {
        ...
        await send
           .SendUsageEventAsync(new ApiUsageEvent(applicationName, context.Request.Path, context.Request.Method, 0, StatusCodes.Status401Unauthorized),
    ...
    private async Task LogAuthorisationFailedAsync(Send send, string applicationName, ForbiddenContext context)
    {
        ...
        await send
           .SendUsageEventAsync(new ApiUsageEvent(applicationName, context.Request.Path, context.Request.Method, 0, StatusCodes.Status403Forbidden),
                                CancellationToken.None);
    }

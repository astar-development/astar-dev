...
public sealed class OneDriveService(ILoginService loginService, ApplicationSettings settings, UserPreferenceService userPreferenceService, ILogger<OneDriveService> logger) : IOneDriveService
{
    ...
    public async Task RunFullSyncAsync(MainWindowViewModel vm, CancellationToken token)
    {
        ...
        var syncManager = new SyncManager(_client, _store, vm, settings, token); // - MOVE THIS OUT OF CONSTRUCTOR
        await syncManager.RunSyncAsync();

        if (userPreferenceService.Load().UiSettings.DownloadFilesAfterSync)
    ...
    public async Task DownloadFilesAsync(MainWindowViewModel vm, CancellationToken token)
    {
        ...
        var reporter = new ProgressReporter(vm, metrics);

        var downloadedIds = new ConcurrentBag<string>();
        using var semaphore = new SemaphoreSlim(settings.MaxParallelDownloads);
        ...
        {
            batch.Add(id);
            if (batch.Count < settings.DownloadBatchSize) continue;
    ...
    /// </summary>
    public async Task<Result<Stream, Exception>> DownloadFileAsync(string path)
    {
        ...
        return await Try.RunAsync(async () =>
        {
            ...
            Stream? stream = await client.Drives[drive.Id].Root.ItemWithPath(normalized).Content.GetAsync();
            return stream!;
        });
    }
    ...
    /// </summary>
    public async Task<Result<DriveItem, Exception>> UploadFileAsync(string path, Stream content)
    {

...
public class AddNewFilesService(IFileSystem fileSystem, IOptions<DatabaseUpdaterConfiguration> config, ILogger<AddNewFilesService> logger)
{
    ...
    public async Task<Result<bool, ErrorResponse>> StartAsync(CancellationToken stoppingToken)
    {
    ...
    private static string GenerateHandleWithoutPrefixes(IFileInfo fileInfo)
        => $"{GenerateRandomNumeric()}-{GenerateFileHandle(fileInfo.Name)}";

    private static string GenerateHandleWithOnePrefix(IFileInfo fileInfo, string prefix)
        => $"{GenerateRandomNumeric()}-{prefix}-{GenerateFileHandle(fileInfo.Name)}";

    private static string GenerateHandleWithTwoPrefixes(IFileInfo fileInfo, string prefix, string prefix2)
        => $"{GenerateRandomNumeric()}-{prefix}-{prefix2}-{GenerateFileHandle(fileInfo.Name)}";

    private static int GenerateRandomNumeric()
        => Random.Shared.Next(5_000, 500_000);

...
public class ClassificationProcessorShould : IDisposable
{
    ...
    public async Task Should_Add_New_Classifications_And_FileNameParts()
    {
        ...
        var appHost       = await DistributedApplicationTestingBuilder.CreateAsync<Program>(stoppingToken);
        ...
        await using var app = await appHost.BuildAsync(stoppingToken).WaitAsync(DefaultTimeout, stoppingToken);
        await app.StartAsync(stoppingToken).WaitAsync(DefaultTimeout, stoppingToken);

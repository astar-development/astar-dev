...
public class PostedFilesHandlerShould
{
    [Fact]
    public async Task ReturnBadRequestWhenTooManyFilesPostedAsync()
    {
        ...
        var response     = await PostedFilesHandler.HandleAsync(new() { FilesToAdd = tooManyFiles }, null!, mockTime, "Mock User", CancellationToken.None);
    ...
    public async Task AddFilesToTheContextAsync()
    {
        ...
        _ = await PostedFilesHandler.HandleAsync(new() { FilesToAdd = [new()] }, mockContext, mockTime, "Mock User", CancellationToken.None);
    ...
    public async Task ReturnTheExpectedResultObjectAsync()
    {
        ...
        var response = await PostedFilesHandler.HandleAsync(new() { FilesToAdd = [new()] }, mockContext, mockTime, "Mock User", CancellationToken.None);
